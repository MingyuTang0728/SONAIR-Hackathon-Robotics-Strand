 ...
 *      (sonair_application_server.py — run by the organiser)
 *   2. GitHub Issues          — if configured, cross-device alternative
 *   3. localStorage           — last resort, same-device only
 *
 * ── SETUP ────────────────────────────────────────────────────────────
 *
 * Option A — Use the organiser's Python server (recommended on the day)
 *
...
<div id="srm-box">
  ...
  <div id="srm-ok">
    ...
    detectBackend().then(mode => {
      ...
      const map   = {server:['⬤ SONAIR server connected','rgba(63,185,80,.7)','Saved to the shared SONAIR database. The campus team will review and respond.'],
                     ...
                     local: ['⚠ local only','rgba(200,100,50,.7)','⚠ Saved locally only — campus team must be on this device to see it. Ask organiser for server IP.']};
  ...
  async function _renderInboxInto(containerId) {
    ...
    const modeNote={
      ...
      local: `<span style="font-size:9px;color:rgba(200,100,50,.7);">⚠ localStorage only — visible on this device only. Ask organiser for server IP (SONAIR_API_BASE).</span>`,
    }[mode]||'';
    const reqHtml=err
